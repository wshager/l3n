...
function serialize($node) {
    ...
    var nil = Buffer.alloc(1),
        ...
        type,
        offsets;
    var getName = function getName(cur) {
        var name = cur.name();
        if (!names[name]) {
            ncount++;
            names[name] = ncount;
            namebuf = Buffer.concat([namebuf, Buffer.from(_fastintcompression2.default.compress(str2array(name))), nil]);
        }
        return ncount;
    };
    var getValue = function getValue(cur) {
        var val = cur.value();
        if (!values[val]) {
            vcount++;
            values[val] = vcount;
            valbuf = Buffer.concat([valbuf, Buffer.from(_fastintcompression2.default.compress(str2array(val))), nil]);
        }
        return vcount;
    };
    ...
    while (cur) {
        ...
        if (type == 1 && index < l) {
            ...
            if (type == 1 && l == 1 && nxt._type == 3) {
                out = Buffer.concat([out, Buffer.from(_fastintcompression2.default.compress([depth + 1, type, getName(cur), getValue(nxt)])), nil]);
                depth--;
                cur = cur._parent;
                if (cur) {
                    index = cur._index + 1;
            ...
            if (type == 1) {
                out = Buffer.concat([out, Buffer.from(_fastintcompression2.default.compress([depth + 1, type, getName(cur)])), nil]);
            } else if (type == 2 || type == 7) {
                out = Buffer.concat([out, Buffer.from(_fastintcompression2.default.compress([depth + 1, type, getName(cur), getValue(cur)])), nil]);
            } else {
                out = Buffer.concat([out, Buffer.from(_fastintcompression2.default.compress([depth + 1, type, getValue(cur)])), nil]);
            }
            // go back up
            depth--;
    ...
    offsets = Buffer.from(_fastintcompression2.default.compress([namebuf.length, valbuf.length]));
...
function parse(buf) {
    ...
    var cur = 0,
        ...
        nodes = [];
    var stack = [];
    var nn, node, name, value, depth, type;

    for (var i = 0; i < buf.length; i++) {
        if (buf[i] === 0) {
            if (!offsets) {
                offsets = _fastintcompression2.default.uncompress(buf.slice(cur, i));
                ...
                //nodes.push();
                nn = _fastintcompression2.default.uncompress(buf.slice(cur, i));
                depth = nn[0];
                type = nn[1];
                //var l = nn.length;
                ...
                if (type == 1 && nn.length > 3) {
                    stack[depth + 1] = [new _xvnode.Node(3, null, null, values[nn[3] - 1])];
                }
                value = type > 1 ? values[nn[type == 2 || type == 7 ? 3 : 2] - 1] : stack[depth + 1] ? stack[depth + 1] : [];
                node = new _xvnode.Node(type, name, null, value);
...
function compress($node) {
    ...
    for (i = 0, l = nodes.length; i < l; i++) {
        out = Buffer.concat([out, Buffer.from(_fastintcompression2.default.compress(nodes[i])), nil]);
    }
    return Buffer.concat([Buffer.from(_fastintcompression2.default.compress(offsets)), nil, out]);
}
    ...
    for (var i = 0; i < buf.length; i++) {
        if (buf[i] === 0) {
            if (!offsets) {
                ...
                nodes.push(_fastintcompression2.default.uncompress(buf.slice(cur, i)));
            }
            cur = i + 1;
        }
    }
