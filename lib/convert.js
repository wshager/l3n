...
function toVNodeStream($s, bufSize = 1) {
  // TODO fix bufSize = 0
  const cx = (0, _vnode.getContext)(this, inode); // create fragment node here; doc constructor expects children
  ...
  return _rxjs.Observable.create($o => {
    $node.subscribe({
      next(vnode) {
        ...
        if (type == 9 || type == 11) return; // NOTE doctype type needs to be serialized as 16, but we handle it in serialization

        $o.next(type);

        if ((0, _type.hasName)(type)) {
          $o.next(vnode.name);
        } else if ((0, _type.isLeaf)(type) || type == 2) {
          $o.next(vnode.value);
        } // emit all pairs
