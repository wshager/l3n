...
function next(inode, indexInParent, type) {
	return _nextOrPrev(inode, indexInParent - 1, type, 1);
}

function previous(inode, indexInParent, type) {
	return _nextOrPrev(inode, indexInParent - 1, type, -1);
}
...
function cached(inode, type) {
	...
	if (type == 1 || type == 9 || type == 11) {
		let children = inode.$children,
		    len = children.length,
		    cache = _multimap2.default.default();
		for (var i = 0; i < len; i++) {
...
function stringify(inode, type, prettyXML = x => x) {
