...
function ensureDoc($node) {
	...
	return just($node).pipe((0, _operators.concatMap)(function (node) {
		if (!node.inode) {
			...
			if (type == 9 || type == 11) {
				...
				return doc.pipe((0, _operators.concatMap)(function (doc) {
		...
		if (typeof node.inode === "function") {
			...
			return d.bind(cx)(node).pipe((0, _operators.concatMap)(function (node) {
				var next = node.first();
...
function _d(type, children) {
	...
	return just(children).pipe((0, _operators.concatMap)(c => (0, _rxjs.isObservable)(c) ? c : (0, _vnode.isVNode)(c) ? just(c) : (0, _construct.x)(c)), (0, _operators.concatMap)(child => child.inode(node)), (0, _operators.reduce)((node, child) => node.modify(child), node));
}
